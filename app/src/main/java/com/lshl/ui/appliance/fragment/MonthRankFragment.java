...
import com.lshl.ui.appliance.adapter.MonthRankAdapter;
...
public class MonthRankFragment extends LazyFragment<FragmentAllrankBinding> {
    ...
    public static MonthRankFragment newInstance() {
        ...
        MonthRankFragment fragment = new MonthRankFragment();
    ...
    private void loadData(int page) {
        RetrofitManager.toSubscribe(ApiClient.getApiService(ApiService.class, RetrofitManager.RetrofitType.Object).updateRankList("month", page + ""), new ProgressSubscriber<>(mContext, new SubscriberOnNextListener<RankBean>() {
            @Override
    ...
    protected void initViews() {
        ...
        mList = new ArrayList<>();
        monthRankAdapter = new MonthRankAdapter(mList);
        mDataBinding.recyclerRank.setAdapter(monthRankAdapter);
    }

    private static class MyHandler extends RefreshHandler {

        private final WeakReference<MonthRankFragment> mFragment;

        public MyHandler(MonthRankFragment fragment) {
