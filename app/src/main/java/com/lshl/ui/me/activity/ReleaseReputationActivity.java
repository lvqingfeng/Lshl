...
import com.lshl.bean.request.ReleaseReputationRequestBean;
import com.lshl.databinding.ActivityReleaseReputationBinding;
 ...
 * 发布口碑
 */
public class ReleaseReputationActivity extends BaseActivity<ActivityReleaseReputationBinding> {
    ...
    public static void actionStart(Activity activity, Constant.ReputationType type, int requestCode) {
        Intent intent = new Intent(activity, ReleaseReputationActivity.class);
    ...
    protected void setupTitle() {
        String titleStr = "口碑发布";
        switch (mReputationType) {
            case RED:
                description = "1";
                titleStr = "红榜发布";
                break;
        ...
        setTextTitleView(titleStr, DEFAULT_TITLE_TEXT_COLOR);
    ...
    protected void initFieldBeforeMethods() {
        ...
        if (intent != null) {
            mReputationType = (Constant.ReputationType) intent.getSerializableExtra("type");
        }
    ...
    protected void initViews() {
        ...
        mDataBinding.setReputationType(mReputationType);
                ...
                DialogUtils.alertDialog(mContext, "温馨提示", "是否确认要提交以上内容"
                        , new SweetAlertDialog.OnSweetClickListener() {
                            ...
                            public void onClick(SweetAlertDialog sweetAlertDialog) {
                                ...
                                String cityName = mDataBinding.tvCityName.getText().toString();
                                String type = mReputationType.getValue();
                                if (type.equals("2")) {
                                    if (mImageList.size() <= 0) {
                                        Toast.makeText(mContext, "请至少添加一张图片", Toast.LENGTH_SHORT).show();
                                        return;
                                    }
                                }
                                if (TextUtils.isEmpty(title) || TextUtils.isEmpty(info)) {
                                    Toast.makeText(mContext, "请将基本信息填写完整", Toast.LENGTH_SHORT).show();
                                    return;
                                }
                                ReleaseReputationRequestBean requestBean = new ReleaseReputationRequestBean();
                                requestBean.setToken(LoginHelper.getInstance().getUserToken());
                                requestBean.setTitle(title);
                                requestBean.setInfo(info);
                                requestBean.setCity(cityName);
                                requestBean.setType(type);
                                Map<String, String> body = ApiClient.createBodyMap(requestBean);
                                updateReputation(body);
                                //checkFileSize(body);
     ...
     * @param body
     */
    private void updateReputation(final Map<String, String> body) {
        if (mImageList.size() <= 0) {
            ...
            RetrofitManager.toSubscribe(ApiClient.getApiService(ApiService.class
                    ...
                    .sendKouBeiHong(body), new Subscriber<HttpResult<String>>() {
                @Override
            ...
            BitmapUtils.pictureCompression(mContext, mImageList, new BitmapUtils.OnPhotoCompressionCallback() {
                ...
                public void onCompressionCallback(final List<String> imageUrl) {
                    ...
                    RetrofitManager.toSubscribe(
                            ...
                            ).releaseReputation(ApiClient.getImageParams("file", imageUrl), body)
    ...
    private void startOpenShare(String projectId) {
        ...
        toNewsBean.onlylabel = Constant.SHARE_KEY_KOUBEI;
        toNewsBean.title = title;
        toNewsBean.content = content;
        toNewsBean.description = description;
    ...
    protected int getLayoutId() {
        return R.layout.activity_release_reputation;
    }
}
