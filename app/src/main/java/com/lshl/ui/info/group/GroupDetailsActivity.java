...
import com.lshl.databinding.ActivityGroupDetailsBinding;
...
public class GroupDetailsActivity extends BaseActivity<ActivityGroupDetailsBinding> implements OnClickListener {
    ...
    private static final int REQUEST_CODE_UPDATE_GROUP_IMAGE = 6;
    ...
    protected void initFieldBeforeMethods() {
        ...
        RelativeLayout changeGroupImageLayout = (RelativeLayout) findViewById(R.id.rl_change_group_image);
        ...
        TextView usersText = (TextView) findViewById(R.id.tv_group_users_value);
        RelativeLayout rl_invite_join = (RelativeLayout) findViewById(R.id.rl_invite_join);
        ...
        usersText.setText(group.getAffiliationsCount() + "/" + CreateNewGroupActivity.MAX_USERS);
        if (group.getOwner() == null || "".equals(group.getOwner())
            ...
            changeGroupImageLayout.setVisibility(View.GONE);
        }
        ...
        if (EMClient.getInstance().getCurrentUser().equals(group.getOwner())) {
            ...
            rl_invite_join.setVisibility(View.VISIBLE);
            rl_invite_join.setOnClickListener(new OnClickListener() {
                @Override
        ...
        adapter = new GridAdapter(this, R.layout.item_group_details_member_list, members);
        ...
        changeGroupImageLayout.setOnClickListener(this);
    }

    @Override
    protected void setupTitle() {
        setTextTitleView(group.getGroupName() + "(" + group.getAffiliationsCount() + st, DEFAULT_TITLE_TEXT_COLOR);
        ...
        if (resultCode == RESULT_OK) {
            ...
            switch (requestCode) {
                ...
                case REQUEST_CODE_EDIT_GROUPNAME: //修改群名称
                            ...
                            public void run() {
                                try {
                                    ...
                                    runOnUiThread(new Runnable() {
                                        public void run() {
                                            setTextTitleView(returnData + "(" + group.getAffiliationsCount()
                                                    + st, DEFAULT_TITLE_TEXT_COLOR);
                    ...
                    break;
                case REQUEST_CODE_UPDATE_GROUP_IMAGE:
                    mBroadcastManager.sendBroadcast(new Intent(Constant.ACTION_GROUP_CHANAGED));
                    break;
                default:
                    break;
    ...
    public void exitGroup(View view) {
        DialogUtils.alertDialog(mContext, "退出群", "你将退出" + group.getGroupName() + "\n （" + groupId + "），退群通知仅群管理员可见。", new SweetAlertDialog.OnSweetClickListener() {
            @Override
    ...
    public void exitDeleteGroup(View view) {
        ...
        DialogUtils.alertDialog(mContext, "解散群", "解散群后你将失去和群友的联系，确认要解散吗？", new SweetAlertDialog.OnSweetClickListener() {
            @Override
            public void onClick(SweetAlertDialog sweetAlertDialog) {
                sweetAlertDialog.dismissWithAnimation();
            }
                ...
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            setResult(RESULT_OK);
                            finish();
                            Toast.makeText(mContext, "退出群聊成功", Toast.LENGTH_SHORT).show();
                        }
                    });
     ...
     * 将删除群的信息更新到后台服务器
     */
    private void deleteGroupUpdate() {

        RetrofitManager.toSubscribe(
                ApiClient.getApiService(
                        ApiService.class
                        , RetrofitManager.RetrofitType.String).deleteGroup(groupId)
                , new ProgressSubscriber<ResponseBody>(mContext, new SubscriberOnNextListener<ResponseBody>() {
                    @Override
                    public void onNext(ResponseBody result) {
                        try {
                            String jsonResult = result.string();
            ...
            public void run() {
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            refreshMembers();
                            setTextTitleView(group.getGroupName() + "(" + group.getAffiliationsCount()
                                    + st, DEFAULT_TITLE_TEXT_COLOR);
                            progressDialog.dismiss();
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            progressDialog.dismiss();
                            Toast.makeText(getApplicationContext(), st6 + "您没有该权限", Toast.LENGTH_LONG).show();
                        }
                    });
                }
            }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.rl_change_group_name:
                EditActivity.actionStart(GroupDetailsActivity.this, "修改群名称", group.getGroupName(), groupId, REQUEST_CODE_EDIT_GROUPNAME);
                break;
            case R.id.rl_change_group_image:
                UpdateGroupImageActivity.actionStart(GroupDetailsActivity.this, groupId, REQUEST_CODE_UPDATE_GROUP_IMAGE);
                break;
            default:
                break;
...
//                EaseUserUtils.setUserShow(username, holder.textView, holder.imageView);
                holder.imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
                EaseUser user = getUserInfo(username);
                //从好友列表中获取，判断如果是空的，或者是头像地址为空，就通过网络获取他的头像和名称
                if (user == null || TextUtils.isEmpty(user.getAvatar())) {
                    getTempUserInfo(username, holder.imageView, holder.textView);


                } else {//否则就去直接设置
                    EaseUserUtils.setUserAvatar(username, holder.imageView);
                ...
                button.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (isInDeleteMode) {
                            ...
                            // user.getUsername()));
                            HxMemberDetailsActivity.actionStart(GroupDetailsActivity.this, username, "", false);
                        }
                    }
                                ...
                                try {
                                    ...
                                    runOnUiThread(new Runnable() {
                                        ...
                                        public void run() {
                                            ...
                                            refreshMembers();
                                            setTextTitleView(group.getGroupName() + "("
                                                    + group.getAffiliationsCount() + st, DEFAULT_TITLE_TEXT_COLOR);
                                        }
                                    });
                                } catch (final Exception e) {
                                    deleteDialog.dismiss();
    ...
    private void getTempUserInfo(String username, final ImageView imageView, final TextView textView) {
        RetrofitManager.toSubscribe(
                ApiClient.getApiService(
