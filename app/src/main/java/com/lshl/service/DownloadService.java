...
import com.lshl.R;
import com.lshl.camera.sd_card_utils.SDCardUtils;
...
/**
 * 用于应用自动更新的服务
 * Created by maxilong on 2017/3/4.
 */
...
public class DownloadService extends Service {
    ...
    private PendingIntent getInstallIntent() {
        File file = new File(SDCardUtils.getFileStoragePath() + mDownloadUrl.substring(mDownloadUrl.lastIndexOf("/")));
    ...
    private void downloadFile(String url) {
        if (SDCardUtils.isCanDownloadFile(SDCardUtils.getFileStoragePath() + mDownloadUrl.substring(
                mDownloadUrl.lastIndexOf("/")))) {

            OkHttpUtils.get().url(url).build().execute(new FileCallBack(SDCardUtils.getFileStoragePath(),
